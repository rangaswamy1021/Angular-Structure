...
export class RecentActivityComponent implements OnInit {
  ...
  assignValuesToRecentActivitiesRequest(getCustomerActivityRequest: ICustomerActivitesRequest): any {
    ...
    let fromDate;
    let toDate;
    if (this.createForm.controls['bsRangeValue'].value == ('' || null)) {
      let todayDate = new Date();
      //let parsedDate = this.datePickerFormatService.getFormattedDateRange(todayDate);
      // strDate = parsedDate;
      fromDate = new Date(todayDate);
      fromDate.setHours(0, 0, 0, 0);
      toDate = new Date(todayDate);
      toDate.setHours(23, 59, 59, 997);
    } else {
      let parsedDate = this.datePickerFormatService.getFormattedDateRange(this.createForm.controls['bsRangeValue'].value);
      strDate = parsedDate;
      fromDate = new Date(strDate[0]);
      toDate = new Date(strDate[1]);
      toDate.setHours(23, 59, 59, 997);
    ...
    this.getCustomerActivityRequest.StartDate = fromDate.toLocaleString(defaultCulture).replace(/\u200E/g,"");
    this.getCustomerActivityRequest.EndDate = toDate.toLocaleString(defaultCulture).replace(/\u200E/g,"");
    ...
    this.getCustomerActivityRequest.SortDir = this.sortingDirection == true ? 1 : 0;
    this.systemActivites = <ISystemActivities>{};
    this.systemActivites.LoginId = this.sessionConstextResponse.loginId;
    this.systemActivites.UserId = this.sessionConstextResponse.userId;
    this.systemActivites.SubSystem = SubSystem[SubSystem.CSC];
  ...
  sortDirection(SortingColumn) {
    ...
    else if (this.sortingColumn == "CREATEDUSER") {
      this.gridArrowCREATEDUSER = true;
      if (this.sortingDirection == true) {
        this.sortingDirection = false;
      }
      else {
